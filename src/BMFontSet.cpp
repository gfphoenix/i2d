...
void BMFontSet::addGlyph(const Glyph &g)
{
//    for(auto const &x : charset_){
//        if(x.code()==g.code()){
//            printf("already exist glyph with code=%x ...\n", g.code());
//            return;
//        }
//    }
//    charset_.push_back(g);
...
const Glyph *BMFontSet::findGlyph(uint16_t ucs16)const
{
    //for(auto const &x : charset_)
    //    if(x.code()==ucs16)
    //       return &x;
...
int BMFontSet::findKerning(uint16_t first, uint16_t second)const
{
...
void BMFontSet::addKerning(uint16_t first, uint16_t second, int amount)
{
//    {
//        if(findKerning(first, second)!=0){
//            fprintf(stderr, "addkerning : already exists??? (%u,%u)\n",
//                    (unsigned)first, (unsigned)second);
//            return;
//        }
//    }
//    kerning_.push_back({makeup32(first, second), amount});
    auto v = makeup32(first, second);
...
bool BMFontSet::parseInfo(BMFontSet &out, const char *p)
{
    const char *q;
    char tmp[128];
...
bool BMFontSet::parseCommon(BMFontSet &out, const char *p)
{
    const char *q;
...
bool BMFontSet::parsePage(BMFontSet &out, const char *p)
{
    const char *q;
...
bool BMFontSet::parseChar(BMFontSet &out, const char *p)
{
...
bool BMFontSet::parseKerning(BMFontSet &out, const char *p)
{
...
BMFontSet *BMFontSet::load(const char *fnt)
{
    ...
    char buffer[1024];
    char *p;
    BMFontSet *tmpObj = MM<BMFontSet>::New();
    BMFontSet &TMP = *tmpObj;
    BMFontSet *out=nullptr;
    int n;
    // parse info
...
out:
    fclose(file);
    MM<BMFontSet>::Del(tmpObj);
    return out;
}

void BMFontSet::Dispose(){}

void BMFontSet::test()
{
//    Ref_ptr<BMFont> f = load("font.fnt");
    Ref_ptr<BMFontSet> f = TextureManager::getInstance()->loadBMFontSet("font.fnt");
    if(f){
        f->dump();
    }else{
        fprintf(stderr, "nil\n");
